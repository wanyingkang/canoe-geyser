...
Real wall1_x1edge, wall1_x2edge, wall2_x1edge, wall2_x2edge;
...
void WallInteraction(MeshBlock *pmb, Real const time, Real const dt,
  ...
  for (int jj = 0; jj <= 1; ++jj) {
    ...
    //if ( (fclose(x2f_left,wall1_x2edge) && (x1f_center < wall1_x1edge) ) || (fclose(x2f_right,wall2_x2edge) || (x1f_center < wall2_x1edge ))  ){
    //if ( ((x2f_left - wall1_x2edge < pmb->pcoord->dx2f(js)) && (x1f_center < wall1_x1edge) && (x2f_center > wall1_x2edge) ) || ((wall2_x2edge - x2f_right < pmb->pcoord->dx2f(je)) || (x1f_center < wall2_x1edge ) && (x2f_center < wall2_x2edge)  )  ){
    if ( (x2f_center < wall1_x2edge) || (x2f_center > wall2_x2edge) ) {
              continue;
          }
    if ( (x1f_center > wall1_x1edge) ) {
              continue;
          }
    if ( (x2f_left - wall1_x2edge < pmb->pcoord->dx2f(js)) || (wall2_x2edge - x2f_right < pmb->pcoord->dx2f(je))) {
  ...
  if (x1s < x1min+pmb->pcoord->dx1f(is)) {
      for (int k = pmb->ks; k <= pmb->ke; ++k)
        for (int j = pmb->js; j <= pmb->je; ++j) {
          ...
          if ((pmb->pcoord->x2v(j) < wall1_x2edge) || pmb->pcoord->x2v(j) > wall2_x2edge) {
              continue;
          }
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  wall1_x1edge = pin->GetReal("mesh","wall1_x1edge");
  wall1_x2edge = pin->GetReal("mesh","wall1_x2edge");
  wall2_x1edge = pin->GetReal("mesh","wall2_x1edge");
  wall2_x2edge = pin->GetReal("mesh","wall2_x2edge");
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  Real wall1_corner_x2 = wall1_x2edge;
  Real wall1_corner_x1 = wall1_x1edge;

  Real wall2_corner_x2 = wall2_x2edge;
  Real wall2_corner_x1 = wall2_x1edge;

  // Enroll wall1, left boundary
